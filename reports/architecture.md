...
2. Ranked nodes by degree + betweenness centrality (`reports/graph_report_annotated.md`).
   The top **code** Hub Nodes are `Task` (deg 43), `CentralPlannerScheduler` (deg 55),
   `Parameter` (deg 42), `Worker` (deg 35), `Target`.
3. The **block diagram** = those Hub Nodes + the real edges between them.
...
- `six` and a vendored `static/` jQuery asset appear as high-degree nodes — **noise** Hub Nodes
  (a compat shim and a bundled JS file), distinct from the *architectural* Hub Nodes. Flagging both,
...
`CRITICAL`/`WARNING` Hub-Node tiers. The architectural hubs (`Task`, `Scheduler`, `Parameter`,
