...
try:
    ...
    logger.warning(
        "Bigquery module imported, but google-api-python-client is "
...
class BigqueryClient(object):
    ...
    def __init__(self, oauth_credentials=None, descriptor="", http_=None):
        ...
        if descriptor:
            self.client = discovery.build_from_document(
                descriptor, credentials=oauth_credentials, http=http_
            )
        else:
            self.client = discovery.build(
                "bigquery", "v2", credentials=oauth_credentials, http=http_
            )
    ...
    def table_exists(self, table):
        ...
        try:
            self.client.tables().get(
                projectId=table.project_id, datasetId=table.dataset_id, tableId=table.table_id
            ).execute()
        except http.HttpError as ex:
            if ex.resp.status == 404:
                return False
    ...
    def make_dataset(self, dataset, raise_if_exists=False, body={}):
        ...
        try:
            self.client.datasets().insert(
                projectId=dataset.project_id,
                body=dict({"id": "{}:{}".format(dataset.project_id, dataset.dataset_id)}, **body),
            ).execute()
        except http.HttpError as ex:
    ...
    def delete_dataset(self, dataset, delete_nonempty=True):
        ...
        self.client.datasets().delete(
            projectId=dataset.project_id,
            datasetId=dataset.dataset_id,
            deleteContents=delete_nonempty,
        ).execute()
...
class BigqueryLoadTask(luigi.Task):
    ...
    def run(self):
        output = self.output()
        assert isinstance(output, BigqueryTarget), "Output should be a bigquery target, not %s" % (
            output
        )
