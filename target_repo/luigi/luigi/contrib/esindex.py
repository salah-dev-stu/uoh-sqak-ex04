...
try:
    ...
    logger.warning(
        "Loading esindex module without elasticsearch installed. "
...
class ElasticsearchTarget(luigi.Target):
    ...
    marker_index = luigi.configuration.get_config().get(
        "elasticsearch", "marker-index", "update_log"
    )
    marker_doc_type = luigi.configuration.get_config().get(
        "elasticsearch", "marker-doc-type", "entry"
    )
    ...
    def ensure_hist_size(self):
        ...
        result = self.es.search(
            index=self.marker_index,
            doc_type=self.marker_doc_type,
            body={"query": {"term": {"target_index": self.index}}},
            sort=("date:desc",),
        )
        ...
        for i, hit in enumerate(result.get("hits").get("hits"), start=1):
            if i > self.marker_index_hist_size:
                ...
                self.es.delete(
                    id=marker_document_id, index=self.marker_index, doc_type=self.marker_doc_type
                )
        self.es.indices.flush(index=self.marker_index)
    ...
    * marker_index_hist_size (0)
    ...
    def run(self):
        ...
        * create entry in marker index.
        ...
        bulk_index(es, self._docs(), chunk_size=self.chunk_size, raise_on_error=self.raise_on_error)
