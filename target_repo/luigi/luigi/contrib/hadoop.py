        ...
        if getattr(package, "__path__", None):
            ...
            if p.endswith(".egg") and os.path.isfile(p):
                ...
                for parent in package.__name__.split(".")[0:-1]:
                    ...
                    add(
                        dereference(
                            __import__(module_name, None, None, "non_empty").__path__[0]
                            + "/__init__.py"
                        ),
    ...
    def run_job(self, job):
        ...
        if self.end_job_with_atomic_move_dir:
            if isinstance(job.output(), luigi.s3.S3FlagTarget):
                raise TypeError("end_job_with_atomic_move_dir is not supported for S3FlagTarget")
            output_hadoop = "{output}-temp-{time}".format(
                output=output_final, time=datetime.datetime.now().isoformat().replace(":", "-")
            )
        else:
        ...
        for target in luigi.task.flatten(job.input_hadoop()):
            if not isinstance(target, luigi.contrib.hdfs.HdfsTarget) and not isinstance(
                target, luigi.s3.S3Target
            ):
                raise TypeError("target must be an HdfsTarget or S3Target")
            arglist += ["-input", target.path]

        if not isinstance(job.output(), luigi.contrib.hdfs.HdfsTarget) and not isinstance(
            job.output(), luigi.s3.S3FlagTarget
        ):
...
class LocalJobRunner(JobRunner):
    ...
    def group(self, input_stream):
        ...
        for i, line in enumerate(input_stream):
            ...
            blob = md5(
                str(i).encode("ascii")
            ).hexdigest()  # pseudo-random blob to make sure the input isn't sorted
...
class JobTask(BaseHadoopJobTask):
    ...
    def run_reducer(self, stdin=sys.stdin, stdout=sys.stdout):
        ...
        outputs = self._reduce_input(
            self.internal_reader((line[:-1] for line in stdin)), self.reducer, self.final_reducer
        )
        self.writer(outputs, stdout)
    ...
    def run_combiner(self, stdin=sys.stdin, stdout=sys.stdout):
        ...
        self.init_combiner()
        outputs = self._reduce_input(
            self.internal_reader((line[:-1] for line in stdin)), self.combiner, self.final_combiner
        )
        self.internal_writer(outputs, stdout)
