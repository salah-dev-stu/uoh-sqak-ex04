...
class HdfsAtomicWritePipe(luigi.format.OutputPipeProcessWrapper):
    ...
    def __init__(self, path):
        ...
        super(HdfsAtomicWritePipe, self).__init__(
            load_hadoop_cmd() + ["fs", "-put", "-", self.tmppath]
        )

    def abort(self):
        logger.info(
            "Aborting %s('%s'). Removing temporary file '%s'",
...
class HdfsAtomicWriteDirPipe(luigi.format.OutputPipeProcessWrapper):
    ...
    def __init__(self, path, data_extension=""):
        ...
        super(HdfsAtomicWriteDirPipe, self).__init__(
            load_hadoop_cmd() + ["fs", "-put", "-", self.datapath]
        )

    def abort(self):
        logger.info(
            "Aborting %s('%s'). Removing temporary dir '%s'",
            self.__class__.__name__,
            self.path,
            self.tmppath,
        )
