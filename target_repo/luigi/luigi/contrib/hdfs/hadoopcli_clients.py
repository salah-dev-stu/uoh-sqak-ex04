...
def create_hadoopcli_client():
    ...
    if version == "cdh4":
        ...
        raise ValueError(
            "Error: Unknown version specified in Hadoop versionconfiguration parameter"
        )
...
class HdfsClient(hdfs_abstract_client.HdfsFileSystem):
    ...
    def count(self, path):
        ...
        for line in stdout.split("\n"):
            if (
                line.startswith("OpenJDK 64-Bit Server VM warning")
                or line.startswith("It's highly recommended")
                or not line
            ):
