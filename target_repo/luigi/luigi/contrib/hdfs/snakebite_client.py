...
class SnakebiteHdfsClient(hdfs_abstract_client.HdfsFileSystem):
    ...
    def chmod(self, path, permissions, recursive=False):
        ...
        return list(self.get_bite().chmod(self.list_path(path), permissions, recursive))
    ...
    def chown(self, path, owner, group, recursive=False):
        ...
        if owner:
            if group:
                return all(
                    bite.chown(self.list_path(path), "%s:%s" % (owner, group), recurse=recursive)
                )
    ...
    def count(self, path):
        ...
        return {"content_size": content_size, "dir_count": dir_count, "file_count": file_count}
    ...
    def mkdir(self, path, parents=True, mode=0o755, raise_if_exists=False):
        ...
        """
        result = list(self.get_bite().mkdir(self.list_path(path), create_parent=parents, mode=mode))
