...
def run_hive(args, check_return_code=True):
    ...
    if check_return_code and p.returncode != 0:
        raise HiveCommandError(
            "Hive command: {0} failed with error code: {1}".format(" ".join(cmd), p.returncode),
            stdout,
            stderr,
        )
    return stdout
...
class HiveCommandClient(HiveClient):
    ...
    def partition_spec(self, partition):
        ...
        return ",".join(
            [
                "`{0}`='{1}'".format(k, v)
                for (k, v) in sorted(six.iteritems(partition), key=operator.itemgetter(0))
            ]
        )
    ...
    def table_schema(self, table, database="default"):
        with HiveThriftContext() as client:
            return [
                ...
                for field_schema in client.get_schema(database, table)
            ]
        ...
        try:
            logger.debug(
                "Checking Hive table '{d}.{t}' for partition {p}".format(
                    d=self.database, t=self.table, p=str(self.partition)
                )
            )
    ...
    def output(self):
        if self.partition is not None:
            ...
            return HivePartitionTarget(
                table=self.table, partition=self.partition, database=self.database
            )
        else:
