    ...
    def run(self):
        ...
        try:
            ...
            if proc.returncode != 0:
                ...
                raise SparkJobError(
                    "Spark job failed {0}".format(repr(args)), out=stdout, err=stderr
                )
        finally:
            tmp_stderr.close()
            tmp_stdout.close()
...
class PySparkTask(SparkSubmitTask):
    ...
    def run(self):
        ...
        self.run_pickle = os.path.join(
            self.run_path, ".".join([self.name.replace(" ", "_"), "pickle"])
        )
...
class SparkJob(luigi.Task):
    ...
    def deps(self):
        ...
        return luigi.task.flatten(self.requires_hadoop()) + luigi.task.flatten(
            self.requires_local()
        )
    ...
    def run(self):
        warnings.warn(
            "The use of SparkJob is deprecated. Please use SparkSubmitTask or PySparkTask.",
            stacklevel=2,
        )
        original_output_path = self.output().path
        path_no_slash = (
            original_output_path[:-2]
            if original_output_path.endswith("/*")
            else original_output_path
        )
        path_no_slash = (
            original_output_path[:-1] if original_output_path[-1] == "/" else path_no_slash
        )
        tmp_output = luigi.contrib.hdfs.HdfsTarget(
            path_no_slash + "-luigi-tmp-%09d" % random.randrange(0, 1e10)
        )
        ...
        for a in self.job_args():
            if a == self.output().path:
                ...
                logger.info(
                    "Using temp path: %s for path %s", tmp_output.path, original_output_path
                )
        ...
        with SparkRunContext(proc) as context:
            while proc.poll() is None:
                ...
                if not app_id:
                    ...
                    elapsed_mins, elapsed_secs = divmod(
                        datetime.timedelta(seconds=time.time() - start).seconds, 60
                    )
...
class Spark1xJob(Spark1xBackwardCompat):
    ...
    def run(self):
        warnings.warn(
            "The use of Spark1xJob is deprecated. Please use SparkSubmitTask or PySparkTask.",
            stacklevel=2,
        )
...
class PySpark1xJob(Spark1xBackwardCompat):
    ...
    def run(self):
        warnings.warn(
            "The use of PySpark1xJob is deprecated. Please use SparkSubmitTask or PySparkTask.",
            stacklevel=2,
        )
