    ...
    def engine(self):
        ...
        if not conn or conn.pid != pid:
            ...
            engine = sqlalchemy.create_engine(
                self.connection_string, connect_args=self.connect_args, echo=self.echo
            )
        ...
        with engine.begin() as con:
            ...
            if not con.dialect.has_table(con, self.marker_table):
                self.marker_table_bound = sqlalchemy.Table(
                    ...
                    sqlalchemy.Column(
                        "inserted", sqlalchemy.DateTime, default=datetime.datetime.now()
                    ),
                )
...
class CopyToTable(luigi.Task):
    ...
    * subclass and override the required `connection_string`, `table` and `columns` attributes.
    ...
    reflect = (
        False  # Set this to true only if the table has already been created by alternate means
    )
    ...
    def create_table(self, engine):
        ...
        needs_setup = (
            (len(self.columns) == 0) or (False in [len(c) == 2 for c in self.columns])
            if not self.reflect
            else False
        )
        if needs_setup:
