...
class RemoteContext(object):
    ...
    def _prepare_cmd(self, cmd):
        connection_cmd = [
            ...
            self._host_ref(),
            "-S",
            "none",  # disable ControlMaster since it causes all sorts of weird behaviour with subprocesses...
        ]
        if self.sshpass:
    ...
    def tunnel(self, local_port, remote_port=None, remote_host="localhost"):
        ...
        proc.communicate()
        assert proc.returncode == 0, "Tunnel process did an unclean exit (returncode %s)" % (
            proc.returncode,
        )
