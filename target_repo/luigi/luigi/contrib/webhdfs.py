...
try:
    ...
    logger.warning(
        "Loading webhdfs module without `hdfs` package installed. "
...
class WebHdfsClient(object):
    ...
    def get_config(self, key):
        ...
        try:
            ...
            raise RuntimeError(
                "You must specify %s in the [hdfs] section of the luigi.cfg file" % key
            )
    ...
    def download(self, hdfs_path, local_path, overwrite=False, n_threads=-1):
        return self.webhdfs.download(
            hdfs_path, local_path, overwrite=overwrite, n_threads=n_threads
        )
    ...
    def read(
        self, hdfs_path, offset=0, length=None, buffer_size=None, chunk_size=1024, buffer_char=None
    ):
        return self.webhdfs.read(
            hdfs_path,
            offset=offset,
            length=length,
