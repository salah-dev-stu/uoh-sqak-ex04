    ...
    def find_all_by_parameters(self, task_name, session=None, **task_params):
        ...
        with self._session(session) as session:
            tasks = (
                ...
                .join(TaskEvent)
                .filter(TaskRecord.name == task_name)
                .order_by(TaskEvent.ts)
                .all()
            )
            for task in tasks:
                if all(
...
class TaskParameter(Base):
    ...
    task_id = sqlalchemy.Column(
        sqlalchemy.Integer, sqlalchemy.ForeignKey("tasks.id"), primary_key=True
    )
    name = sqlalchemy.Column(sqlalchemy.String(128), primary_key=True)
    value = sqlalchemy.Column(sqlalchemy.String(256))

    def __repr__(self):
        return "TaskParameter(task_id=%d, name=%s, value=%s)" % (
            self.task_id,
            self.name,
            self.value,
        )
...
class TaskEvent(Base):
    ...
    def __repr__(self):
        return "TaskEvent(task_id=%s, event_name=%s, ts=%s" % (
            self.task_id,
            self.event_name,
            self.ts,
        )
