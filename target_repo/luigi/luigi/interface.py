...
class core(task.Config):
    ...
    local_scheduler = parameter.BoolParameter(default=False, description="Use local scheduling")
    ...
    lock_size = parameter.IntParameter(
        default=1, description="Maximum number of workers running the same command"
    )
    no_lock = parameter.BoolParameter(
        default=False, description="Ignore if similar process is already running"
    )
    ...
    parallel_scheduling = parameter.BoolParameter(
        default=False, description="Use multiprocessing to do scheduling in parallel."
    )
...
class Interface(object):
    ...
    def run(tasks, worker_scheduler_factory=None, override_defaults=None):
        ...
        if not env_params.no_lock and not (
            lock.acquire_for(env_params.lock_pid_dir, env_params.lock_size)
        ):
            sys.exit(1)
...
def add_global_parameters(parser):
    for task_name, is_without_section, param_name, param in get_global_parameters():
        param.add_to_cmdline_parser(
            parser, param_name, task_name, glob=True, is_without_section=is_without_section
        )
...
def set_global_parameters(args):
    ...
    for task_name, is_without_section, param_name, param in get_global_parameters():
        param.set_global_from_args(
            param_name, task_name, args, is_without_section=is_without_section
        )
