...
class Parameter(object):
    ...
    * Any default value set using the ``default`` flag.
    ...
    def __init__(
        ...
        self.is_bool = (
            is_boolean and not is_list
        )  # Only BoolParameter should ever use this. TODO(erikbern): should we raise some kind of exception?
        if is_global:
            warnings.warn(
                "is_global support is removed. Assuming positional=False",
                DeprecationWarning,
                stacklevel=2,
            )
            positional = False
        ...
        if config_path is not None and ("section" not in config_path or "name" not in config_path):
            raise ParameterException(
                "config_path must be a hash containing entries for section and name"
            )
        ...
        if not x:
            if self.has_task_value(param_name=param_name, task_name=task_name):
                ...
                raise MissingParameterException(
                    "No value for '%s' (%s) submitted and no default value has been assigned."
    ...
    def add_to_cmdline_parser(
        self, parser, param_name, task_name, glob=False, is_without_section=False
    ):
        ...
        parser.add_argument(flag, help=" ".join(description), action=action, dest=dest)
    ...
    def set_global_from_args(self, param_name, task_name, args, is_without_section=False):
        ...
        dest = self.parser_dest(
            param_name, task_name, glob=True, is_without_section=is_without_section
        )
        if dest is not None:
            value = getattr(args, dest, None)
            if value:
...
class TimeDeltaParameter(Parameter):
    ...
    def _parseSimple(self, input):
        ...
        regex = "".join(
            ["((?P<%s>\d+) ?%s(%s)?(%s)? ?)?" % (k, k[0], k[1:-1], k[-1]) for k in keys]
        )
        return self._apply_regex(regex, input)
