...
try:
    ...
    logger.warning(
        "Loading s3 module without boto installed. Will crash at "
...
class S3Client(FileSystem):
    ...
    def __init__(self, aws_access_key_id=None, aws_secret_access_key=None, **kwargs):
        ...
        for key in ["aws_access_key_id", "aws_secret_access_key"]:
            if key in options:
                options.pop(key)
        self.s3 = boto.connect_s3(aws_access_key_id, aws_secret_access_key, **options)
        ...
        try:
            ...
            for i in range(num_parts):
                ...
                with open(local_path, "rb") as fp:
                    part_num = i + 1
                    logger.info(
                        "Uploading part %s/%s to %s", part_num, num_parts, destination_s3_path
                    )
                    fp.seek(offset)
...
class S3FlagTarget(S3Target):
    ...
    * the _SUCCESS file exists within the directory.
    ...
    def __init__(self, path, format=None, client=None, flag="_SUCCESS"):
        ...
        if path[-1] != "/":
            raise ValueError(
                "S3FlagTarget requires the path to be to a "
