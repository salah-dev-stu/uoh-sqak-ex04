...
class scheduler(Config):
    ...
    disable_window = parameter.IntParameter(
        default=3600, config_path=dict(section="scheduler", name="disable-window-seconds")
    )
    disable_failures = parameter.IntParameter(
        default=None, config_path=dict(section="scheduler", name="disable-num-failures")
    )
    disable_hard_timeout = parameter.IntParameter(
        default=None, config_path=dict(section="scheduler", name="disable-hard-timeout")
    )
    disable_persist = parameter.IntParameter(
        default=86400, config_path=dict(section="scheduler", name="disable-persist-seconds")
    )
    ...
    visualization_graph = parameter.Parameter(
        default="svg", config_path=dict(section="scheduler", name="visualization-graph")
    )
...
class Task(object):
    ...
    def can_disable(self):
        return self.disable_failures is not None or self.disable_hard_timeout is not None
...
class SimpleTaskState(object):
    ...
    def load(self):
        if os.path.exists(self._state_path):
            ...
            logger.info(
                "No prior state file exists at %s. Starting with clean slate", self._state_path
            )
    ...
    def get_pending_tasks(self):
        return itertools.chain.from_iterable(
            six.itervalues(self._status_tasks[status]) for status in [PENDING, RUNNING]
        )
        ...
        if new_status == FAILED and task.can_disable():
            ...
            if task.has_excessive_failures():
                ...
                notifications.send_error_email(
                    "Luigi Scheduler: DISABLED {task} due to excessive failures".format(
                        task=task.id
                    ),
    ...
    def prune(self, task, config, assistants):
        ...
        if not task.stakeholders:
            if task.remove is None:
                logger.info(
                    "Task %r has stakeholders %r but none remain connected -> will remove "
                    "task in %s seconds",
                    task.id,
                    task.stakeholders,
                    config.remove_delay,
        ...
        if (
            ...
            and task.worker_running not in task.stakeholders | assistants
        ):
            logger.info(
                "Task %r is marked as running by disconnected worker %r -> marking as "
                "FAILED with retry delay of %rs",
                task.id,
                task.worker_running,
                config.retry_delay,
            )
    ...
    def get_necessary_tasks(self):
        ...
        for task in self.get_active_tasks():
            if (
                task.status not in (DONE, DISABLED)
                or getattr(task, "scheduler_disable_time", None) is not None
            ):
...
class CentralPlannerScheduler(Scheduler):
    ...
    def prune(self):
        ...
        for worker in self._state.get_active_workers():
            if worker.prune(self._config):
                logger.info(
                    "Worker %s timed out (no contact for >=%ss)",
                    worker,
        ...
        for task in self._state.get_active_tasks():
            if task.id not in necessary_tasks and self._state.prune(
                task, self._config, assistant_ids
            ):
    ...
    def add_task(
        ...
        * update priority when needed
        ...
        task = self._state.get_task(
            task_id,
            setdefault=self._make_task(
                task_id=task_id,
                status=PENDING,
                deps=deps,
                resources=resources,
        ...
        for task in tasks:
            ...
            if self._schedulable(task) and self._has_resources(task.resources, greedy_resources):
                if in_workers and self._has_resources(task.resources, used_resources):
                    ...
                    workers = (
                        itertools.chain(task.workers, [worker_id]) if assistant else task.workers
                    )
            ...
            while task_stack:
                ...
                if self._state.has_task(dep_id):
                    ...
                    if dep_id not in upstream_status_table:
                        ...
                        upstream_status = [
                            upstream_status_table.get(task_id, "") for task_id in dep.deps
                        ]
    ...
    def task_list(self, status, upstream_status, limit=True, search=None, **kwargs):
        ...
        for task in filter(filter_func, self._state.get_active_tasks(status)):
            if (
                ...
                or upstream_status == self._upstream_status(task.id, upstream_status_table)
            ):
