...
class Register(abc.ABCMeta):
    ...
    def _get_reg(cls):
        ...
        for cls in cls._reg:
            ...
            if (
                name in reg
                and reg[name] != cls
                and reg[name] != cls.AMBIGUOUS_CLASS
                and not issubclass(cls, reg[name])
            ):
    ...
    def _missing_task_msg(cls, task_name):
        weighted_tasks = [
            (Register._editdistance(task_name, task_name_2), task_name_2)
            for task_name_2 in cls.task_names()
        ]
