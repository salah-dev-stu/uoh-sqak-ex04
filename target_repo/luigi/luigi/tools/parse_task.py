...
def id_to_name_and_params(task_id):
    ...
    parameter = (
        ...
        + (
            ...
            + pp.ZeroOrMore(pp.Word(value_chars) + pp.ZeroOrMore(pp.Literal(",")).suppress())
            + (pp.Literal(")").suppress() | pp.Literal("]").suppress())
        )
    ).setResultsName("list_params", listAllMatches=True) | (
        pp.Word(name_chars) + pp.Literal("=").suppress() + pp.Word(value_chars)
    ).setResultsName("params", listAllMatches=True)
