...
class RangeEvent(
    luigi.Event
):  # Not sure if subclassing currently serves a purpose. Stringly typed, events are.
    """
...
class RangeBase(luigi.WrapperTask):
    ...
    now = luigi.IntParameter(
        default=None, description="set to override current time. In seconds since epoch"
    )
    ...
    def _emit_metrics(self, missing_datetimes, finite_start, finite_stop):
        ...
        datetimes = self.finite_datetimes(
            ...
            else min(finite_start, self.parameter_to_datetime(self.start)),
            finite_stop
            if self.stop is None
            else max(finite_stop, self.parameter_to_datetime(self.stop)),
        )

        delay_in_jobs = (
            len(datetimes) - datetimes.index(missing_datetimes[0])
            if datetimes and missing_datetimes
            else 0
        )
    ...
    def requires(self):
        ...
        finite_start = (
            moving_start
            if self.start is None
            else max(self.parameter_to_datetime(self.start), moving_start)
        )
        moving_stop = self.moving_stop(now)
        finite_stop = (
            moving_stop
            if self.stop is None
            else min(self.parameter_to_datetime(self.stop), moving_stop)
        )

        datetimes = (
            self.finite_datetimes(finite_start, finite_stop) if finite_start <= finite_stop else []
        )

        task_cls = self.of_cls
        if datetimes:
            logger.debug(
                "Actually checking if range %s of %s is complete",
                self._format_range(datetimes),
                self.of_cls.task_family,
            )
            missing_datetimes = sorted(self.missing_datetimes(task_cls, datetimes))
            logger.debug(
                "Range %s lacked %d of expected %d %s instances",
                self._format_range(datetimes),
                len(missing_datetimes),
                len(datetimes),
                self.of_cls.task_family,
            )
        else:
        ...
        if required_datetimes:
            logger.debug(
                "Requiring %d missing %s instances in range %s",
                len(required_datetimes),
                self.of_cls.task_family,
    ...
    def missing_datetimes(self, task_cls, finite_datetimes):
        ...
        """
        return [
            d for d in finite_datetimes if not task_cls(self.datetime_to_parameter(d)).complete()
        ]
...
class RangeDailyBase(RangeBase):
    ...
    stop = luigi.DateParameter(
        default=None, description="ending date, exclusive. Default: None - work forward forever"
    )
...
class RangeHourlyBase(RangeBase):
    ...
    stop = luigi.DateHourParameter(
        default=None, description="ending datehour, exclusive. Default: None - work forward forever"
    )
    ...
    def finite_datetimes(self, finite_start, finite_stop):
        ...
        datehour_start = datetime(
            finite_start.year, finite_start.month, finite_start.day, finite_start.hour
        )
        datehours = []
        for i in itertools.count():
...
def _get_per_location_glob(tasks, outputs, regexes):
    ...
    matches = [
        r.search(p) for r, p in zip(regexes, paths)
    ]  # naive, because some matches could be confused by numbers earlier in path, e.g. /foo/fifa2000k/bar/2000-12-31/00

    for m, p, t in zip(matches, paths, tasks):
        if m is None:
            raise NotImplementedError(
                "Couldn't deduce datehour representation in output path %r of task %s" % (p, t)
            )

    n_groups = len(matches[0].groups())
    positions = [
        most_common((m.start(i), m.end(i)) for m in matches)[0] for i in range(1, n_groups + 1)
    ]  # the most common position of every group is likely to be conclusive hit or miss
    ...
    return "".join(
        ...
        0
    ]  # chop off the last path item (wouldn't need to if `hadoop fs -ls -d` equivalent were available)
...
class RangeDaily(RangeDailyBase):
    ...
    def missing_datetimes(self, task_cls, finite_datetimes):
        try:
            return set(finite_datetimes) - set(
                map(
                    self.parameter_to_datetime,
                    task_cls.bulk_complete(map(self.datetime_to_parameter, finite_datetimes)),
                )
            )
    ...
    def missing_datetimes(self, task_cls, finite_datetimes):
        try:
            return set(finite_datetimes) - set(
                map(
                    self.parameter_to_datetime,
                    task_cls.bulk_complete(list(map(self.datetime_to_parameter, finite_datetimes))),
                )
            )
        except NotImplementedError:
            return infer_bulk_complete_from_fs(
