...
def common_params(task_instance, task_cls):
    ...
    common_param_names = list(
        set.intersection(set(task_instance_param_names), set(task_cls_param_names))
    )
    ...
    def __call__(self, task_that_copies):
        ...
        class Wrapped(task_that_copies):
            def run(_self):
                ...
                f = o.open(
                    "w"
                )  # TODO: assert that i, o are Target objects and not complex datastructures
...
def previous(task):
    ...
    if len(previous_date_params) == 0:
        ...
        raise NotImplementedError(
            "Too many date-related task parameters - can't determine previous task"
        )
    else:
