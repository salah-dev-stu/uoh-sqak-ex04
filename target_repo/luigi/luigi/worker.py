...
class TaskProcess(multiprocessing.Process):
    ...
    def run(self):
        ...
        try:
            ...
            status = FAILED
            logger.exception(
                "[pid %s] Worker %s failed    %s", os.getpid(), self.worker_id, self.task
            )
...
class worker(Config):
    ping_interval = FloatParameter(
        default=1.0, config_path=dict(section="core", name="worker-ping-interval")
    )
    keep_alive = BoolParameter(
        default=False, config_path=dict(section="core", name="worker-keep-alive")
    )
    ...
    wait_interval = IntParameter(
        default=1, config_path=dict(section="core", name="worker-wait-interval")
    )
    max_reschedules = IntParameter(
        default=1, config_path=dict(section="core", name="worker-max-reschedules")
    )
    timeout = IntParameter(default=0, config_path=dict(section="core", name="worker-timeout"))
    task_limit = IntParameter(
        default=None, config_path=dict(section="core", name="worker-task-limit")
    )
...
class Worker(object):
    ...
    * asks for stuff to do (pulls it in a loop and runs it)
    ...
    def __init__(
        self, scheduler=None, worker_id=None, worker_processes=1, assistant=False, **kwargs
    ):
    ...
    def _add(self, task, is_complete):
        if (
            self._config.task_limit is not None
            and len(self._scheduled_tasks) >= self._config.task_limit
        ):
            logger.warning(
                "Will not schedule %s or any dependencies due to exceeded task-limit of %d",
                task,
        ...
        if is_complete:
            ...
            task.trigger_event(Event.DEPENDENCY_MISSING, task)
            logger.warning(
                "Data for %s does not exist (yet?). The task is an external data depedency, so it can not be run from this luigi process.",
                task.task_id,
            )
    ...
    def _log_remote_tasks(self, running_tasks, n_pending_tasks, n_unique_pending):
        ...
        if running_tasks:
            ...
            logger.info(
                "There are %s pending tasks possibly being run by other workers", n_pending_tasks
            )
            if n_unique_pending:
    ...
    def _purge_children(self):
        ...
        for task_id, p in six.iteritems(self._running_tasks):
            if not p.is_alive() and p.exitcode:
                error_msg = "Worker task %s died unexpectedly with exit code %s" % (
                    task_id,
                    p.exitcode,
                )
            elif (
                p.timeout_time is not None and time.time() > float(p.timeout_time) and p.is_alive()
            ):
                p.terminate()
        ...
        while True:
            ...
            try:
                task_id, status, error_message, missing, new_requirements = (
                    self._task_result_queue.get(timeout=float(self._config.wait_interval))
                )
            except Queue.Empty:
                return
            ...
            if new_requirements:
                new_req = [
                    load_task(module, name, params) for module, name, params in new_requirements
                ]
                for t in new_req:
                    self.add(t)
    ...
    def run(self):
        ...
        while True:
            while len(self._running_tasks) >= self.worker_processes:
                logger.debug(
                    "%d running tasks, waiting for next task to finish", len(self._running_tasks)
                )
